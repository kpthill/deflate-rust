...
impl HuffmanTree {
    ...
    /// (sym, len)
    pub fn next_symbol(&self, bits: u16, max_len: u8) -> Result<(Symbol, u8)> {
...
impl CodePoint {
    fn new(bits: u16, len: u8) -> CodePoint {
        let mask = (1 << len) - 1;
        CodePoint {
            bits: bits & mask,
            len,
...
impl SpecCodePoint {
    fn new(bits: u16, len: u8) -> SpecCodePoint {
...
// fn decode_distance(CodePoint { bits, len }: CodePoint) -> Result<Distance> {
//     ensure!(len == 5); // assuming fixed trees for now
//     Ok(distance_mapping(bits as u8))
// }

// fn distance_mapping(code_point: u8) -> Distance {
//     let (num_bits, base) = match code_point {
//         0 => (0, 1),
//         1 => (0, 2),
//         2 => (0, 3),
//         3 => (0, 4),
...
//         _ => unreachable!(),
//     };
//     Distance { num_bits, base }
// }

/// (i, num_bits, base)
const HUFFMAN_LENGTH_MAPPING_7_BIT: [(u16, u8, u16); 23] = [
    (257, 0, 3),
    (258, 0, 4),
    (259, 0, 5),
    (260, 0, 6),
...
/// (i, num_bits, base)
const HUFFMAN_LENGTH_MAPPING_8_BIT: [(u16, u8, u16); 6] = [
    (280, 4, 115),
    (281, 5, 131),
    (282, 5, 163),
    (283, 5, 195),
